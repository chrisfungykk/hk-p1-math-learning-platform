...
export default function ExamPrepMode() {
    ...
    return (
      <div className="flex flex-col items-center gap-4 sm:gap-6 py-4 px-2">
        <div className="flex items-center gap-3 w-full max-w-3xl">
          ...
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-red-700">📝 考試準備 — {semester.name}</h2>
        </div>
        <div className="flex flex-col items-center gap-4 w-full max-w-md px-2">
          <p className="text-lg sm:text-xl font-bold text-gray-700">選擇難度</p>
          <div className="grid grid-cols-2 sm:flex gap-3 w-full">
            {(['easy', 'medium', 'hard', 'challenge'] as DifficultyLevel[]).map((level) => (
              <button key={level} onClick={() => handleDifficultyChange(level)}
                className={`min-h-14 sm:min-h-16 min-w-12 sm:flex-1 rounded-2xl text-base sm:text-lg font-bold shadow-md transition-all ${
                  difficulty === level
          ...
          <button onClick={handleStartExam}
            className="min-h-14 min-w-12 w-full rounded-2xl bg-gradient-to-r from-red-400 to-pink-400 text-white text-lg sm:text-xl font-bold py-4 shadow-lg hover:scale-105 transition-transform mt-2 sm:mt-4">
            📝 開始考試準備
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center gap-3 sm:gap-4 py-3 sm:py-4 px-1 sm:px-2">
      {/* Header */}
      <div className="flex items-center gap-3 w-full max-w-3xl px-1">
        <Link to="/"
          className="min-h-12 min-w-12 flex items-center justify-center rounded-2xl bg-gray-200 hover:bg-gray-300 text-2xl transition-colors"
          aria-label="返回主頁">⬅️</Link>
        <h2 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-red-700">📝 考試準備 — {semester.name}</h2>
      </div>

      {/* Exam paper */}
      <div className="w-full max-w-3xl bg-white rounded-2xl sm:rounded-3xl shadow-lg p-3 sm:p-4 md:p-6 border-2 border-red-200">
        {/* Paper header */}
        <div className="text-center border-b-2 border-gray-300 pb-3 mb-3 sm:mb-4">
          <h3 className="text-lg sm:text-xl md:text-2xl font-extrabold text-gray-800">
        ...
        {/* Score banner */}
        {showResults && summary && (
          <div className="flex flex-col items-center gap-2 sm:gap-3 bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl p-3 sm:p-4 mb-3 sm:mb-4 border border-purple-200">
            <div className="flex items-center gap-3 sm:gap-4">
              <span className="text-3xl sm:text-4xl">{getScoreEmoji(percentage)}</span>
              <div className="text-center">
                <p className="text-2xl sm:text-3xl font-extrabold text-purple-700">{percentage}%</p>
                <p className="text-xs sm:text-sm text-gray-600">✅ {summary.correct} 正確 ｜ ❌ {summary.incorrect} 錯誤 ｜ 共 {summary.total} 題</p>
              </div>
            </div>
            {/* Per-topic breakdown */}
            {topicBreakdown.length > 0 && (
              <div className="w-full grid grid-cols-2 md:grid-cols-3 gap-1.5 sm:gap-2 mt-1 sm:mt-2">
                {topicBreakdown.map((tb) => {
                  ...
                  return (
                    <div key={tb.topicId} className="flex items-center justify-between bg-white rounded-xl shadow-sm px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm">
                      <span className="font-bold text-gray-700 truncate">{topicName}</span>
                      <span className={`font-bold ml-1 sm:ml-2 ${topicPct >= 80 ? 'text-green-600' : topicPct >= 50 ? 'text-yellow-600' : 'text-red-500'}`}>
                        {tb.correct}/{tb.total}
                      </span>
                    </div>
                  );
        ...
        {topicGroups.map((group, groupIdx) => (
          <div key={group.topicId} className="mb-4 sm:mb-6">
            <div className="flex items-center gap-2 mb-2 sm:mb-3 border-b border-gray-200 pb-2">
              <span className="text-xs sm:text-sm font-bold text-white bg-red-400 rounded-full px-2.5 sm:px-3 py-0.5 sm:py-1">
                第{['甲', '乙', '丙', '丁', '戊', '己'][groupIdx] ?? (groupIdx + 1)}部
              </span>
              <span className="text-sm sm:text-base font-bold text-gray-700">{group.topicName}</span>
            </div>
            ...
            <div className="flex flex-col gap-2.5 sm:gap-3">
              {group.questions.map(({ q, globalIdx }) => {
                ...
                return (
                  <div key={q.id} className={`rounded-xl sm:rounded-2xl p-2.5 sm:p-3 border-2 transition-colors ${
                    showResults
                    ...
                    <div className="flex gap-2 mb-2">
                      <span className="flex-shrink-0 w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center rounded-full bg-red-100 text-red-700 font-bold text-xs">
                        {globalIdx + 1}
                      </span>
                      <p className="text-sm sm:text-base font-bold text-gray-800 whitespace-pre-line flex-1">{q.prompt}</p>
                    </div>
                    {q.illustration && <div className="ml-8 sm:ml-9 mb-1"><QuestionIllustration svg={q.illustration} /></div>}

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2 ml-8 sm:ml-9">
                      {q.options.map((option, oIdx) => {
                        let btnClass = 'min-h-11 sm:min-h-9 rounded-xl text-sm font-semibold py-2 sm:py-1.5 px-3 transition-all text-left flex items-center gap-2 ';
                        if (showResults) {
                    ...
                    {showResults && selected !== undefined && !isCorrect && (
                      <p className="text-xs text-gray-600 mt-2 ml-8 sm:ml-9 bg-yellow-50 rounded-lg p-2">💡 {q.explanation}</p>
                    )}
                    {showResults && selected === undefined && (
                      <p className="text-xs text-red-500 mt-2 ml-8 sm:ml-9">⚠️ 未作答</p>
                    )}
                  </div>
                );
              })}
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 mt-4 pt-4 border-t-2 border-gray-200">
          {!showResults ? (
            <button onClick={handleSubmit}
              className="min-h-14 min-w-12 rounded-2xl bg-gradient-to-r from-red-400 to-pink-400 text-white text-lg font-bold px-8 py-3 shadow-lg hover:scale-105 transition-transform w-full sm:w-auto">
              📝 交卷 ({answeredCount}/{questions.length})
            </button>
          ) : (
            <>
              <Link to="/"
                className="min-h-14 min-w-12 flex items-center justify-center rounded-2xl bg-gradient-to-r from-blue-400 to-cyan-400 text-white text-lg font-bold px-6 py-3 shadow-lg hover:scale-105 transition-transform">
                🏠 返回主頁
              </Link>
              <button onClick={handleRetry}
                className="min-h-14 min-w-12 flex items-center justify-center rounded-2xl bg-gradient-to-r from-red-400 to-pink-400 text-white text-lg font-bold px-6 py-3 shadow-lg hover:scale-105 transition-transform">
                🔄 再試一次
              </button>
            </>
          )}
